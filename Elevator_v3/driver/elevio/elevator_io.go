...
const _pollRate = 180 * time.Millisecond
...
func TurnOffLights(floors int) {
	_mtx.Lock()
	defer _mtx.Unlock()
	for i := 0; i < floors; i++{
		_conn.Write([]byte{2, byte(1), byte(i), toByte(false)})
		_conn.Write([]byte{2, byte(0), byte(i), toByte(false)})
	}
}
